...
| macOS Intel | Python 3.10-3.12 CI 与 CLI 冒烟测试通过 | 自动化验证通过，真机待验证 |
| macOS Apple Silicon | Python 3.10-3.12 CI 与 CLI 冒烟测试通过 | 自动化验证通过，真机待验证 |

仓库分别使用 `macos-latest`（arm64）和 `macos-15-intel`（x86_64）运行测试。两种架构均已验证 Python/Bash 入口、UTF-8 中文路径、安装与回滚、发布包权限，以及固定版本 Lark/Feishu CLI 和 Whiteboard CLI 的安装、版本与帮助命令。真实 Mac 上的用户登录和飞书文档读写仍待验证，因此暂不标记为完整真机支持。
...
Windows 还会检查 PowerShell 入口。macOS CI 会在 Intel 与 Apple Silicon runner 上执行 Bash 语法、中文路径安装、tar.gz 权限、安装后自测，并安装固定版本的两个飞书 CLI 完成冒烟检查。公开 CI 不保存飞书账号，真实文档读写只做手动集成测试。

`Release package smoke` 工作流用于验证已经发布的安装包。它会在 Windows、Intel Mac 和 Apple Silicon Mac 上下载 Release 附件、核对 SHA-256、检查压缩包安全与脚本权限，并在中文路径中完成安装、离线预检和安装后自测。
...
运行失败时先保留原始报错，再按平台定位。Windows 重点检查 PowerShell 解析、`.cmd` 包装、`LOCALAPPDATA`、路径转义、执行策略和 UTF-8；macOS 重点检查 CPU 架构、PATH 或 Homebrew 位置、执行权限和 UTF-8。两边都要继续检查 CLI 版本、用户登录和文档权限。属于项目代码的问题要修正后重跑失败步骤、预检、安装后自测、对应平台的冒烟测试和完整 CI，直到同一问题得到验证。

修复过程中不会通过切换身份、把可编辑画板换成图片、放宽校验规则或跨平台套用专用路径掩盖问题，也不会静默修改用户的 shell 配置、PowerShell 策略、Homebrew、凭证和飞书授权。外部依赖或权限缺失时，预检会给出检测结果和复检命令，依赖恢复后再继续。
...
- macOS Intel 与 Apple Silicon 自动化测试已经通过，但尚未完成两种架构真机上的用户登录和真实飞书文档读写验证。
